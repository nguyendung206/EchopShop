...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="align-items-center">
		<h1 class="h3"><strong>Đối tác</strong></h1>
	</div>
</div>
<div class="filter">
        ...
        <table class="table aiz-table mb-0 table_repon">
            ...
            <tbody>
                ...
                @foreach ($partners as $key => $partner)
                    <tr class="text-center">
                        <td class="font-weight-800 align-middle">{{ ($key + 1) + ($partners->currentPage() - 1) * $partners->perPage() }}</td>
                        <td class="font-weight-400 align-middle">
                            <img style="height: 90px;" class="profile-user-img img-responsive img-bordered" src="{{ getImage($partner->photo) }}">
                        </td>
                        <td class="font-weight-400 align-middle text-overflow">{{optional($partner)->company_name}}</td>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            title: '@lang("Xóa đối tác")',
            text: '@lang("Bạn có muốn xóa đối tác này không ?")',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(err) {
                        ...
                        Swal.fire('Đã xảy ra lỗi!', 'Không thể xóa đối tác.', 'error');
                    }
                });
            }
        });
