...
<div class="filter">
    <form class="" id="food" action="{{ route('admin.category.index') }}" method="GET">
        ...
        <div class="row gutters-5 mb-3 custom-change">
            <div class="col-md-3 ">
                <input type="date" class="form-control custom-placeholder" name="joined_date" id=""
                ...
                <tr class="text-center">
                    ...
                    <td class="text-right">
                        ...
                        <a class="btn mb-1 btn-soft-danger btn-icon btn-circle btn-sm btn_status changeStatus"
                            data-id="{{ $data->id }}"
                            data-href="{{ route('admin.category.changestatus', ['id' => $data->id]) }}"
                        ...
                        <a class="btn mb-1 btn-soft-primary btn-icon btn-circle btn-sm"
                            href="{{ route('admin.category.edit', $data->id) }}" title="@lang('Update')">
...
<script type="text/javascript">
    $(document).on('click', '.changeStatus', function() {
        ...
        let confirmText = status == 1 ? '@lang("Bạn muốn vô hiệu hóa Danh mục này?")' : '@lang("Bạn muốn kích hoạt Danh mục này?")';
    ...
    $(document).on('click', '.btn-delete', function() {
        ...
        Swal.fire({
            title: '@lang("Xóa Danh mục")',
            text: '@lang("Bạn có muốn xóa Danh mục này không ?")',
            icon: 'warning',
            confirmButtonText: '@lang("Có")',
            cancelButtonText: '@lang("Không")',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(err) {
                        Swal.fire('Đã xảy ra lỗi!', 'Không thể xóa Danh mục.', 'error');
                    }
                });
            }
        });
