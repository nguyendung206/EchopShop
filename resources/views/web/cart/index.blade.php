            ...
            <tbody>
                ...
                <tr class="text-center">
                    ...
                    <td class="total-amount align-middle td-amount-{{$cart->id}}">
                        {{ format_price($cart->products->price * $cart->quantity) }}
                    </td>
                    <td class="align-middle">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.typeProductSubmit', function() {
            ...
            const oldQuantity = $(`.quantity-${cartId}`).val();
            const newQuantity = oldQuantity > quantityShow ? quantityShow : oldQuantity;
            $.ajax({
                url: url,
                method: 'PUT',
                data: {
                ...
                success: function(response) {
                    ...
                    $(`.td-quantity-${cartId}`).html(
                        ...
                        `<div class="number-input number-input-${cartId}" data-cartid="${cartId}">
                                    ...
                                    <input class="quantity-${cartId} quantity" type="number" data-url="${urlInput}" data-productunitid="${cart.product_unit_id}" data-cartid="${cart.id}" value="${newQuantity }" min="1"
                                        max="100">
                                    <button class="plus-${cart.id} plus" data-cartid="${cart.id}">+</button>
                                </div>` : `<p class="text-danger">Hết hàng</p>`
                    )
                    toastr.success("Áp dụng loại hàng thành công", null, {
