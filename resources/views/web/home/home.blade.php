    ...
    <div class="secondhand">
        <div class="container">
            <div class="row justify-content-between align-items-center">
                <div class=" pt-2">
                    <div class="container">
                        <br>
                        <h1 class="title-secondhand">Hàng bán</h1>
                    </div>
                </div>
        ...
        <div class="container">
                        ...
                        <a href="{{ route('web.productdetail.index', ['slug' => $product->slug]) }}">
                            ...
                            <div class="user-product-wrap d-flex align-items-center">
                                ...
                                <div class="user-product ">
                                    <p class="line-clamp-1">{{$product->shop->name}} &nbsp;<img src="{{asset('/img/icon/doc-top.png')}}" alt="">&nbsp; {{$product->shop->province?->province_name}}</p>
                                </div>
                                @else
            ...
            <div class="text-center py-5 divMoreSecondhand btn-more-wrap">
                @if ($saleProducts->count() >= 5 && $saleProducts->hasMorePages())
                <a id="btnMoreSale" class="all color-B10000 btn-more" href="#">Xem thêm sản phẩm mới<i
                        class="fa-solid fa-angles-right"></i></a>
                @endif
                <a class="all color-B10000 btn-more" href="{{ route('listProducts', ['type' => TypeProductEnums::SALE]) }}">Xem tất cả <i
                        class="fa-solid fa-angles-right"></i></a>
            </div>
            @endif
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#btnMoreSale').click(function(event) {
            event.preventDefault();
            currentSalePage++;
            ...
            $.ajax({
                ...
                method: 'GET',
                data: {
                    salePage: currentSalePage
                },
                success: function(response) {
                    var productsHtml = '';

                    $('.sale-list').append(response.products);
                    if (response.hasMorePage) {
                        $('#btnMoreSale').hide();
                    }
                },
